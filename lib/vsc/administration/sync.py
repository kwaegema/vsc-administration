...
VSC_TIER1C_STORAGE = ['dodrioscratch']
...
class VscTier1cSync(Sync):
    ...
    def process_t1_users(self, accounts, quotas, storage_name, dry_run=False):
        ...
        this method should not be used and swill be done by process_projects members method
        ...
        for quota in quotas:
            ...
            try:
                user = VscTier1cUser(quota.user, storage_name=storage_name,
                        rest_client=self.apc, use_user_cache=True, dry_run=dry_run)
                user.set_quota()
    ...
    def process_projects(self, prj_ids, storage_name, datestamp, dry_run=False):
        ...
        for prj_id in sorted(prj_ids):
            ...
            prj = VscTier1cProject(prj_id, storage_name=storage_name, rest_client=self.apc, dry_run=dry_run)
            ...
            try:
                ...
                for member in modified_members:
                    try:
                        prj.set_member_quota(member)  # half of the project quota
                        prj.create_member_dir(member)
    ...
    def do(self, dry_run):
        ...
        if self.options.user:
            ...
            for storage_name in VSC_TIER1C_STORAGE:
        ...
        if self.options.projects:
            ...
            for storage_name in VSC_TIER1C_STORAGE:
                ...
                log.info("Found %d %s projects that have changed quota in the accountpage since %s" %
