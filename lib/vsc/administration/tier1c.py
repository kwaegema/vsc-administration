...
from vsc.config.base import GENT, VSC, VscStorage
...
class Dodrio(Storage):
    ...
    def __init__(self):
        super().__init__(
                ...
                # Two new fields without hard dependency on gpfs
                fsbackend='lustre',
...
class VscStorageBackend(object):
    ...
    def set_project_quota(self, pjname, quota=None):
        ...
        self._set_quota(path, pjname, quota)
...
class VscTier1cUser(VscAccountPageUser):
    ...
    #FIXME: implement functions to have home on scratch?
...
class VscTier1cProject(VscAccountPageVo):
    ...
    def __init__(self, pjid, storage_name=None, storage=None, rest_client=None, host_institute=GENT, dry_run=False):
        ...
        self.backend = VscStorageBackend(storage=storage, path_templates=path_templates, dry_run=dry_run)
    ...
    def create_fileset(self):
        ...
        try:
            ...
            log.error("There is no moderator available for %s", self.vo.vsc_id)
            fileset_owner_id = pwd.getpwnam('nobody').pw_uid
        try:
    ...
    def modified_members(self, datestamp):
        ...
        #TODO: projects in accountpage?
        return self.rest_client.vo[self.pjid].member.modified[datestamp].get()

    def create_member_scratch_dir(self, member):
        """ Create a directory for a member in the project """
        pass
